...
curllist=`curl -L -s -H  'User-Agent: Test' https://rammb-slider.cira.colostate.edu/data/json/goes-${4}/full_disk/geocolor/${date1}_by_hour.json | jq -r '.timestamps_int | .[] | .[]' | sort -n`
ARRINDEX=0
declare -a DATEARRAY
while read line; do
	if [ ! -d ${date2}/${line} ]
...
then
	...
	then
		...
		url=${base_url_him}/$1/$2/$3/goes-${4}---full_disk
