...
exec 200>${HOME}/locks/hima_taiwan_${date2}_lock 
flock -xn 200 || exit 1
...
then
	...
	then
		...
		do
			...
			curl -L -f -s --retry 5 --retry-delay 5 --retry-max-time 60 --create-dirs -H 'User-Agent: Test' $url/geocolor/{${chunk}}/04/[002-005]_[003-008].png -o download/${date2}/#1/#2_#3.png
			...
			echo -ne "\rht-$date2\t`ls -1 ${date2} | wc -l` (+${counter})"
			rm -rf download/${date2}
		done
	fi
